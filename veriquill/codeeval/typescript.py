...
# For counting code. Literals go too, so a keyword quoted in a message is not a
# branch and a brace inside a string does not move where a function body ends.
_NOISE = re.compile(f"{_COMMENT}|{_LITERAL}", re.DOTALL)

# For reading what the code says. Literals stay, because a hardcoded credential
# and an interpolated shell command live inside them, and blanking those is
# precisely how you fail to find them. Comments still go: prose about a password
# is not a password.
...
def check_typescript_complexity(
    ...
    return [
        Finding(
            ...
            rationale=(
                f"{len(offenders)} TypeScript or JavaScript function(s) reach a branch "
                f"count of {COMPLEXITY_THRESHOLD} or more. The highest is "
                f"{worst[0][1].name} at {worst[0][1].complexity}. Counted from decision "
                "keywords rather than a parsed syntax tree, so treat it as close rather "
                "than exact."
            ),
    ...
    if source_files and not test_files:
        return [
            Finding(
                ...
                rationale=(
                    f"{len(source_files)} TypeScript or JavaScript source file(s) and no "
                    "test files. Untested code is not necessarily incorrect, but nothing "
                    "here demonstrates that it works."
                ),
                confidence=LEXICAL_CONFIDENCE,
                evidence=(
                    EvidenceRef(
                        ...
                        detail="no *.test.*, *.spec.* or __tests__ files in the repository",
                    ),
                ),
            )
        ]
    ...
    for name, pattern, severity, explanation in _SECURITY_PATTERNS:
        ...
        findings.append(
            Finding(
                ...
                rationale=(
                    f"{len(hits)} occurrence(s) in TypeScript or JavaScript: {explanation}. "
                    "Found by reading the source as text, so confirm the context before "
